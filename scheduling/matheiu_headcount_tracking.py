...
def cumulative_with_built_in_feature(model: cp_model.CpModel, variables: Variables, tasks: Set[str]):
    """
    Built-in cumulative. Only constraint an upper bound
    """
...
def cumulative_with_start_time(model: cp_model.CpModel, variables: Variables, tasks: Set[str]):
    """
    Custom cumulative based on start time.
    Complexity coming from a variable duration
    """
    ...
    for task in tasks:
        ...
        for t in time_range:
            # Capture start time
            model.Add(variables.var_task_starts[task] == t).OnlyEnforceIf(variables.var_task_starts_presence[task, t])
...
def cumulative_with_overlap(model: cp_model.CpModel, variables: Variables, tasks: Set[str]):
    """
...
if __name__ == '__main__':
    ...
    # variables.var_task_starts = {task: model.NewIntVarFromDomain(domain=cp_model.Domain.FromValues(),
    #                                                              name=f"task_{task}_start")
    #                              for task in tasks}
    ...
    # add cumulative
    cumulative_with_start_time(model, variables, tasks)
    # cumulative_with_overlap(model, variables, tasks)
    # cumulative_with_built_in_feature(model, variables, tasks)

    model.AddNoOverlap(variables.var_intervals.values())
    ...
    if print_result:
        ...
        if status == cp_model.OPTIMAL:
            ...
            print(df)
            #print('resource_needs', solution.resource_needs['all'])
